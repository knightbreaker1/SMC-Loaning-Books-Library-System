    ...
    <div class="mainwrapper">
        <div class="leftpanel">
            ...
            <ul class="nav nav-pills nav-stacked">
                ...
                <li class="active"><a href="return_book.php"><i class="fa fa-cube"></i> <span>Return Book</span></a></li>
    ...
    <div class="contentpanel">
        ...
        <thead>
            <tr>
                ...
                <?php
                $count = 0;
                $book = mysqli_query($conn, "SELECT * FROM book as b,borrow_book as bb, student as s where b.book_id=bb.book_id and s.student_id=bb.student_id and bb.status='Borrowed Succesfully' order by borrow_book_id desc");
                ...
                if(mysqli_num_rows ($book) > 0){
                    while($row = mysqli_fetch_array($book)){
                        ...
                        echo '</td>';
                            ...
                            echo '<td>';
                            echo "<a href='return_book.php?return_book=".$row['borrow_book_id']."&book=".$row['book_id']."' onclick='return confirm(\"Are you sure you want to return this book?\")'>Return</a>";
                            echo '</td>';
...
if(isset($_GET['return_book']) != "") {
    $borrow_book_id = $_GET['return_book'];
    $book_id = $_GET['book'];
    ...
    $borrow_book = mysqli_query($conn, "select * from borrow_book where borrow_book_id='$borrow_book_id'");
    $row_borrow_book = mysqli_fetch_array($borrow_book);

    $date_borrow = $row_borrow_book['date_borrow'];

    $d_borrow = new DateTime($date_borrow);
    $now = new DateTime($date);

    $days = $now->diff($d_borrow)->format("%a");
    ...
    $update = mysqli_query($conn, "update borrow_book set status='Return Succesfully',return_book_date='$date',return_book_time='$time',fines='$fines' where borrow_book_id='$borrow_book_id'");

    if($update) {
        echo "<script>alert('Success');location.replace('return_book.php');</script>";
    }
    else {
